...
description: Primitives for text, images, videos, tool calls, and other message content.
---
...
Message parts are usually one of text, reasoning, image, video, audio, tool-call, or data.
...
Video content that can be played back from a URL.
...
Renders the current video message part as a `<video>` element with controls and
metadata preload.

```tsx
import { MessagePartPrimitive } from "@assistant-ui/react";

<MessagePartPrimitive.Video className="max-w-sm rounded-xl" />;
```
