...
<MessagePrimitive.Parts>
  {({ part }) => {
    ...
    if (part.type === "video") return <MyVideoRenderer />;
...
<MessagePrimitive.Parts
  components={{
    ...
    Video: () => <MessagePartPrimitive.Video className="max-w-sm rounded-xl" />,
...
- **`MessagePartPrimitive.Video`**: renders the video of a video part
