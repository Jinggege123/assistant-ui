...
const CodeHeader: FC<CodeHeaderProps> = ({ language, code }) => {
  ...
  const { themeId, setThemeId } = useCodeHighlightTheme();
  ...
  return (
    <div className="aui-code-header-root mt-2.5 flex items-center justify-between rounded-t-lg border border-slate-700 border-b-0 bg-slate-950 px-3 py-1.5 text-xs">
      <span className="aui-code-header-language font-medium text-slate-300 lowercase">
        {language}
      </span>
      <div className="aui-code-header-actions flex items-center gap-2">
        <select
          aria-label="Code highlight theme"
          value={themeId}
          onChange={(event) =>
            setThemeId(event.currentTarget.value as CodeHighlightThemeId)
          }
          className="h-7 rounded-md border border-slate-700 bg-slate-900 px-2 text-xs text-slate-200 outline-none transition-colors hover:bg-slate-800 focus-visible:border-slate-500"
        >
          {codeHighlightThemes.map((theme) => (
        ...
        <button
          ...
          onClick={onCopy}
          className="inline-flex size-7 items-center justify-center rounded-md text-slate-300 transition-colors hover:bg-slate-800 hover:text-white focus-visible:border-slate-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-500/40"
        >
          {!isCopied && <CopyIcon />}
          {isCopied && <CheckIcon />}
        </button>
...
const useCopyToClipboard = ({
  ...
  const copyToClipboard = async (value: string) => {
    ...
    const copied =
      copyWithTemporaryTextArea(value) || (await copyWithClipboardApi(value));
...
const copyWithTemporaryTextArea = (value: string) => {
...
const defaultComponents = memoizeMarkdownComponents({
  ...
  pre: ({ className, ...props }) => (
    <pre
      className={cn(
        "aui-md-pre overflow-x-auto rounded-t-none rounded-b-lg border border-slate-700 border-t-0 p-3 text-xs leading-relaxed",
        className,
      )}
      {...props}
    />
